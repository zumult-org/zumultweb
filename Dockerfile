...
LABEL org.opencontainers.image.source=https://github.com/zumult-org/zumultweb
LABEL org.opencontainers.image.authors="Anne Ferger"
...
ADD . $HOME
WORKDIR $HOME
RUN mvn package -X -s docker-config/settings.xml "-Dproject.packaging=war" "-Dgithub.token=${GITHUB_PAT}" "-Dgithub.username=${GITHUB_USERNAME}"
